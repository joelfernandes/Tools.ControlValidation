...
{
    public partial class OnDemanExampleValidationForm : Form, IUIValidator
    {
        ...
        public OnDemanExampleValidationForm()
        {
            InitializeComponent();
            
            Validator = new Validator(this);
            Validator.ValidStateChange += Validator_ValidStateChange;
            ...
            _radioButtonMyMomThinks.CheckedChanged += (sender, e) => ValidateGroupBox();
            _radioButtonGoodBoy.CheckedChanged += (sender, e) => ValidateGroupBox();
            _radioButtonNobelPrize.CheckedChanged += (sender, e) => ValidateGroupBox();
            _radioButtonPirateArgh.CheckedChanged += (sender, e) => ValidateGroupBox();

            _dateTimePicker.ValueChanged += (sender, e) => ValidateDateTimePicker();
            _comboBox.SelectedIndexChanged += (sender, e) => ValidateComboBox();
            _checkedListBox.ItemCheck += (sender, e) => this.BeginInvoke(new MethodInvoker(ValidateCheckedListBox));
            _checkBox.CheckedChanged += (sender, e) => ValidateCheckBox();

            ValidateAll();
        }

        private void Validator_ValidStateChange(object sender, ValidationStateEventArgs e)
        {
            _buttonSubmit.Enabled = !Validator.HasErrors;
        }
